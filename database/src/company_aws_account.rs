...
#[diesel(table_name = company_aws_accounts)]
pub struct CompanyAwsAccount {
    pub id: String,
    pub company_id: i32,
    pub aws_account_alias: String,
    pub approvals_required: i32,
    pub admin_approval_required: bool,
}

impl From<CompanyAwsAccount> for AwsAccount {
    fn from(a: CompanyAwsAccount) -> Self {
        Self {
            id: a.id,
            account: a.aws_account_alias,
...
impl CompanyAwsAccount {
    ...
    pub fn read(connection: &mut SqliteConnection, id: &str) -> Result<Self> {
        company_aws_accounts::dsl::company_aws_accounts
            .find(id)
            .first(connection)
    }
    ...
    pub fn read_by_alias(
        ...
        company_aws_accounts::table
            .filter(company_aws_accounts::dsl::company_id.eq(company_id))
            .filter(company_aws_accounts::dsl::aws_account_alias.eq(name))
            .first(connection)
    }
    ...
    pub fn delete(&self, connection: &mut SqliteConnection) -> Result<usize> {
        use company_aws_accounts::dsl;

        diesel::delete(dsl::company_aws_accounts.filter(dsl::id.eq(&self.id))).execute(connection)
    }
}
